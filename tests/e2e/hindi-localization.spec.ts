...
test.describe("Hindi localization audit", () => {
  for (const viewport of viewports) {
    test(`${viewport.name} public routes stay localized and viewport-safe`, async ({
      ...
      await installClientPreferences(context, { locale: "hi", theme: "light" });
      await installSupabaseMock(page);
      await seedHindi(page);

      for (const route of publicRoutes) {
        await page.goto(route.path);
        await assertHindiViewportSafe(page, route.path, viewport.name);
      }
    });
    ...
    test(`${viewport.name} authenticated routes stay localized and viewport-safe`, async ({
      ...
      await installAuthenticatedSession(context, { locale: "hi", theme: "light" });
      await installSupabaseMock(page);
      await seedHindi(page);
...
async function seedHindi(page: import("@playwright/test").Page) {
  await page.goto("/");
  await page.evaluate(() => window.localStorage.setItem("astrosaathi.lang", "hi"));
}

async function assertHindiViewportSafe(
